...
#3b. Very quickly you realize that entering descriptions for each actor is too much effort. Delete the description column.actor
...
SELECT s.staff_id AS 'Staff Id',s.first_name AS 'First Name', s.last_name AS 'Last Name', SUM(p.amount) AS Total_Amount
FROM staff s
...
SELECT  c.first_name AS 'First Name', c.last_name AS 'Last Name',SUM(p.amount) AS 'Total Paid by each Customer' 
FROM customer c
INNER JOIN payment p
